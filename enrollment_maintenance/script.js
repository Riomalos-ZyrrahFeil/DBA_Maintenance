...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchJSON(url, options = {}) {
        try {
            ...
            if (err instanceof SyntaxError) {
                 console.error("Failed to parse JSON. Server response was likely HTML/PHP error messages.");
            }
    ...
    updateBtn.addEventListener('click', async () => {
        ...
        if (resp.status === "success") {
            ...
            console.error('rror updating enrollment: ' + resp.message);
    ...
    function getFormData() {
        ...
        if (!student_id || !section_id || !date_enrolled || !status) {
            alert("Please fill in all required fields (Student, Section, Date, Status).");
            return null;
        }
